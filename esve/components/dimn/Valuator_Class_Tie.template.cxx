
#include <esve/components/dimn/Valuator_Class_Tie.hxx>
#include <esve/components/dimn/Valuator_Creator.hxx>
#include <esve/components/dimn/Valuator.hxx>

namespace esve { namespace components { namespace dimn {

template< typename T_Target,
          typename T_Value_Type >
Valuator_Class_Tie<T_Target,
                   T_Value_Type>::
Valuator_Class_Tie( Valuator_Creator<Value_Type> & ui,
                    Target & data )
    : super(),
      m_ui(ui),
      m_target(data),
      m_map()
{
}

template< typename T_Target,
          typename T_Value_Type >
Valuator_Class_Tie<T_Target,
                   T_Value_Type>::
~Valuator_Class_Tie()
{
}

template< typename T_Target,
          typename T_Value_Type >
Valuator<T_Value_Type> &
Valuator_Class_Tie<T_Target,
                   T_Value_Type>::
create_valuator( Reader reader, Writer writer, const std::string & name )
{
    Valuator<Value_Type> & valuator =
        m_ui.create_valuator((m_target.*reader)(), name) ;
    m_map.insert(m_Pair(&valuator, m_Node(reader, writer))) ;
    return valuator ;
}

template< typename T_Target,
          typename T_Value_Type >
void
Valuator_Class_Tie<T_Target,
                   T_Value_Type>::
tie( Valuator<Value_Type> & valuator, Reader reader, Writer writer )
{
    valuator.value((m_target.*reader)()) ;
    m_map.insert(m_Pair(&valuator, m_Node(reader, writer))) ;
}

template< typename T_Target,
          typename T_Value_Type >
bool
Valuator_Class_Tie<T_Target,
                   T_Value_Type>::
handle_valuator( const Valuator<Value_Type> & valuator )
{
    bool handled = false ;

    // const_cast is harmless since we just use the pointer value
    // without dereferencing.
    m_Map_const_iterator i =
        m_map.find(const_cast<Valuator<Value_Type>*>(&valuator)) ;

    if( i != m_map.end() )
    {
        (m_target.*((*i).second.writer))(valuator.value()) ;
        handled = true ;
    }

    return handled ;
}

////////////////////////////////////////////////////////////////
//
// real specialization
//
////////////////////////////////////////////////////////////////

template< typename T_Target >
Valuator_Class_Tie<T_Target, types::real>::
Valuator_Class_Tie( Valuator_Creator<types::real> & ui,
                    Target & data )
    : super(),
      m_ui(ui),
      m_target(data),
      m_map()
{
}

template< typename T_Target >
Valuator_Class_Tie<T_Target, types::real>::
~Valuator_Class_Tie()
{
}

template< typename T_Target >
Valuator<types::real> &
Valuator_Class_Tie<T_Target, types::real>::
create_valuator( Reader reader, Writer writer, const std::string & name )
{
    Valuator<types::real> & valuator =
        m_ui.create_valuator((m_target.*reader)(), name) ;
    m_map.insert(m_Pair(&valuator, m_Node(reader, writer))) ;
    return valuator ;
}

template< typename T_Target >
void
Valuator_Class_Tie<T_Target, types::real>::
tie( Valuator<types::real> & valuator, Reader reader, Writer writer )
{
    valuator.value((m_target.*reader)()) ;
    m_map.insert(m_Pair(&valuator, m_Node(reader, writer))) ;
}

template< typename T_Target >
bool
Valuator_Class_Tie<T_Target, types::real>::
handle_valuator( const Valuator<types::real> & valuator )
{
    bool handled = false ;

    // const_cast is harmless since we just use the pointer value
    // without dereferencing.
    m_Map_const_iterator i =
        m_map.find(const_cast<Valuator<types::real>*>(&valuator)) ;

    if( i != m_map.end() )
    {
        (m_target.*((*i).second.writer))(valuator.value()) ;
        handled = true ;
    }

    return handled ;
}

}}} // namespace esve::components::dimn
